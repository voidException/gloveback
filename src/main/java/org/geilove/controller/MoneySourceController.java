...
public class MoneySourceController {
    ...
    public @ResponseBody Object getMoneySourceList(@RequestBody MoneySourceParam moneySourceParam ){
        //根据受助人的ID获取这个人的"支持了"列表
        DynamicRsp dynamicRsp=new DynamicRsp(); //最终要返回的
        List<Dynamic> listDynamic=new ArrayList<>(); //DynamicRsp 的一个数据域
        ...
        for (int i=0;i<moneySourceIDs.size();i++){ //能走到这一步，说明必然有数据
            ...
            try {
                ...
                //moneySrcPingLunList至少是一次的
                if (moneySrcPingLunList==null){
                    moneySrcPingLunList.get(0).setPingluntext("加油");
                }
            }catch (Exception e){
                moneySrcPingLunList.get(0).setPingluntext("加油");
            }
